...
# Legacy data contains known calendar gaps; train.py intentionally skips them.
# The phase boundary itself must be readable so that checkpoint 20260817 exists.
...
# Fixed test must run before 0829-0831 are consumed to create the rolling seed.
...
# Only after fixed evaluation, consume 0829-0831 and build the 0831 rolling seed.
if ! checkpoint_ready "$auto_test_start_ckpt_day"; then
    run_train_range "$new_train_hdfs_dir" "$test_start_day" \
        "$auto_test_start_ckpt_day" "$train_end_day"
fi

bash rolling_test.sh
echo "matched fixed + rolling protocol completed"
