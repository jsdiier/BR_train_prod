...
class Learner:
    ...
    def train_step_with_saliency(self, feat):
        ...
        gradient_pairs = [(gradient, weight)
        ...
        if self.ema_vars is not None:
            for ema_v, weight in zip(self.ema_vars, model.trainable_weights):
                ema_v.assign(self.ema_decay * ema_v +
        ...
        if model_conf.saliency_collect_enabled:
            ...
            if len(registered_slots) != model_conf.saliency_expected_slot_count:
                raise RuntimeError(
                    ...
                    (len(registered_slots), model_conf.saliency_expected_slot_count))
        ...
        for step, feat in enumerate(train_data):
            ...
            if train_writer is not None and self.gstep % 100 == 0:
                ...
                with train_writer.as_default():
                    ...
                    tf.summary.scalar('data/new_feature_sample_rate', new_feature_sample_rate, step=global_step)
                    tf.summary.scalar('data/interest_group_active_rate', interest_group_active_rate, step=global_step)

                print(datetime.datetime.now(),
                        "day %s steps: %d, buy loss: %04f, pos: %d, cnt: %d,  cat loss: %04f, cat pos: %d,  click loss: %04f, click pos: %d,  ext loss: %04f, ext pos: %d, sampled: %d, new_feature_sample_rate: %.6f, interest_group_active_rate: %.6f" % (
                        ...
                        new_feature_sample_rate, interest_group_active_rate))
    ...
    def dump_saliency_assets(self, start_day, end_day, source_checkpoint):
        ...
        top_rows = sorted(ascending[expected - remain:],
        ...
        if not os.path.isdir('log'):
            os.makedirs('log')
        importance_path = 'log/fea_importance_saliency_%s_%s.tsv' % (
            start_day, end_day)
