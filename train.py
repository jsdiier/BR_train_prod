...
class Learner:
    def __init__(self):
        ...
        self.semantic_diagnostic_path = None
    ...
    def train_step(self, feat, buy_weight=1.0, cat_weight=1.0, click_weight=1.0, ext_weight=1.0):
        ...
        semantic_diagnostics = model.rankmixer.diagnostics(
            model.last_semantic_token_mask)
        return (loss_buy, loss_cat, loss_click, loss_ext, final_loss,
                pred_buy, pred_cat, pred_click, pred_ext) + semantic_diagnostics
    ...
    def train(self, data_arg, start_day, end_day, model_path=None, data_path=None, dump_serving_model=True):
        ...
        self.semantic_diagnostic_path = os.path.join(
            out_dir, 'semantic_domain_rankmixer_diagnostics.tsv')
        if (not os.path.exists(self.semantic_diagnostic_path) or
                os.path.getsize(self.semantic_diagnostic_path) == 0):
            with open(self.semantic_diagnostic_path, 'w') as handle:
                handle.write(
                    'day\tglobal_step\tresidual_ratio\toffdiag_ratio\t'
                    'active_tokens\tactive_fraction\n')
    ...
    def train_one_day(self, train_data, day, train_writer, mfout=None):
        ...
        for step, feat in enumerate(train_data):
            ...
            (loss_buy, loss_cat, loss_click, loss_ext, final_loss,
             ...
             semantic_residual_ratio, semantic_offdiag_ratio,
             semantic_active_tokens,
             semantic_active_fraction) = self.train_step(feat)
            ...
            if train_writer is not None and self.gstep % 100 == 0:
                ...
                with train_writer.as_default():
                    ...
                    tf.summary.scalar('semantic_rankmixer/residual_ratio', semantic_residual_ratio, step=global_step)
                    tf.summary.scalar('semantic_rankmixer/offdiag_ratio', semantic_offdiag_ratio, step=global_step)
                    tf.summary.scalar('semantic_rankmixer/active_tokens', semantic_active_tokens, step=global_step)
                    tf.summary.scalar('semantic_rankmixer/active_fraction', semantic_active_fraction, step=global_step)
                ...
                with open(self.semantic_diagnostic_path, 'a') as handle:
                    handle.write("%s\t%d\t%.8f\t%.8f\t%.4f\t%.8f\n" % (
                        day, self.gstep,
                        float(semantic_residual_ratio.numpy()),
                        float(semantic_offdiag_ratio.numpy()),
                        float(semantic_active_tokens.numpy()),
                        float(semantic_active_fraction.numpy())))
