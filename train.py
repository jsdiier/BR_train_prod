...
class Learner:
    ...
    def save_checkpoint(self, day):
        ...
        # then synchronizes only task-private towers and prediction heads.
        # Embeddings and the shared representation remain online.
        ...
        for ema_v, w in zip(self.ema_vars, model.trainable_weights):
            if id(w) in embedding_ids:
                ...
                w.assign(ema_v)
                task_private_sync_count += 1
            else:
                shared_count += 1
        partition_count = embedding_count + task_private_sync_count + shared_count
        if (embedding_count == 0 or task_private_sync_count == 0 or
                shared_count == 0 or
            ...
            raise RuntimeError(
                ...
                (embedding_count, task_private_sync_count, shared_count,
                 partition_count, len(model.trainable_weights)))
        ...
        print(datetime.datetime.now(), "saved full-EMA evaluation checkpoint for day %s -> %s" %
              (day, save_dir))
        print(datetime.datetime.now(),
              "saved task-private-sync continuation checkpoint for day %s "
              "-> %s (embeddings_online=%d shared_online=%d "
              "task_private_synced=%d)" %
              (day, continuation_save_dir, embedding_count, shared_count,
               task_private_sync_count))
