...
class Learner:
    def __init__(self):
        ...
        self.short_seq_diagnostic_path = None
    ...
    def train_step(self, feat, buy_weight=1.0, cat_weight=1.0, click_weight=1.0, ext_weight=1.0):
        ...
        return (loss_buy, loss_cat, loss_click, loss_ext, final_loss,
                ...
                model.last_short_seq_residual_ratio,
                model.last_short_seq_active_fraction)
    ...
    def train(self, data_arg, start_day, end_day, model_path=None, data_path=None, dump_serving_model=True):
        ...
        self.short_seq_diagnostic_path = os.path.join(
            out_dir, 'short_seq_mean_residual_diagnostics.tsv')
        if (not os.path.exists(self.short_seq_diagnostic_path) or
                os.path.getsize(self.short_seq_diagnostic_path) == 0):
            with open(self.short_seq_diagnostic_path, 'w') as handle:
                handle.write('day\tglobal_step\tresidual_ratio\tactive_fraction\n')
    ...
    def train_one_day(self, train_data, day, train_writer, mfout=None):
        ...
        for step, feat in enumerate(train_data):
            ...
            (loss_buy, loss_cat, loss_click, loss_ext, final_loss,
             pred_buy, pred_cat, pred_click, pred_ext,
             short_seq_residual_ratio,
             short_seq_active_fraction) = self.train_step(feat)
            ...
            if train_writer is not None and self.gstep % 100 == 0:
                ...
                with train_writer.as_default():
                    ...
                    tf.summary.scalar('short_seq_mean_residual/ratio', short_seq_residual_ratio, step=global_step)
                    tf.summary.scalar('short_seq_mean_residual/active_fraction', short_seq_active_fraction, step=global_step)
                ...
                with open(self.short_seq_diagnostic_path, 'a') as handle:
                    handle.write("%s\t%d\t%.8f\t%.8f\n" % (
                        day, self.gstep,
                        float(short_seq_residual_ratio.numpy()),
                        float(short_seq_active_fraction.numpy())))
