...
assert_zero005_day() { local day=$1; $hadoop fs -test -e "${zero005_hdfs_dir}/${day}/_SUCCESS" || { echo "zero005 data day is incomplete: $day"; exit 1; }; }
assert_zero005_range() { local day=$1; local end_day=$2; while [[ "$day" -le "$end_day" ]]; do assert_zero005_day "$day"; day=$(date_next "$day"); done; }

run_train_range() {
    local data_root=$1 start_day=$2 end_day=$3 restore_day=${4:-} stamp
...
run_test_window() {
    local ckpt_day=$1 start_day=$2 end_day=$3 kind=$4
...
if ! checkpoint_ready "$legacy_train_end_day"; then run_train_range "$legacy_train_hdfs_dir" "$train_start_day" "$legacy_train_end_day"; fi
assert_zero005_range "$zero005_train_start_day" "$train_end_day"
if ! checkpoint_ready "$train_end_day"; then run_train_range "$zero005_hdfs_dir" "$zero005_train_start_day" "$train_end_day" "$legacy_train_end_day"; fi
assert_zero005_range "$test_start_day" "$test_end_day"
run_test_window "$train_end_day" "$test_start_day" "$test_end_day" fixed
if ! checkpoint_ready "$auto_test_start_ckpt_day"; then run_train_range "$zero005_hdfs_dir" "$(date_next "$train_end_day")" "$auto_test_start_ckpt_day" "$train_end_day"; fi
...
while [[ "$test_day" -le "$auto_test_end_day" ]]; do
    ...
    if [[ "$test_day" -lt "$auto_test_end_day" ]] && ! checkpoint_ready "$test_day"; then run_train_range "$zero005_hdfs_dir" "$test_day" "$test_day" "$ckpt_day"; fi
    ckpt_day=$test_day
