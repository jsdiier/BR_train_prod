...
def main():
    ...
    experiment_variables = [
        ...
        if 'finalmlp_' in variable.name.lower()]
    experiment_missing = [
        variable.name for variable, gradient
        in zip(model.trainable_variables, gradients)
        if 'finalmlp_' in variable.name.lower() and gradient is None]
    experiment_gradients = [
        variable.name for variable, gradient
        in zip(model.trainable_variables, gradients)
        if 'finalmlp_' in variable.name.lower() and gradient is not None]
    if not experiment_variables:
        raise RuntimeError('experiment variables received no gradients: finalmlp_')
    if experiment_missing:
        raise RuntimeError(
            'experiment variables with missing gradients: %s' %
            experiment_missing)

    diagnostics = model.finalmlp_two_stream.diagnostics()
    print('FINALMLP_GATE user_mean=%.6f item_mean=%.6f user_saturation=%.6f item_saturation=%.6f' % (
        float(diagnostics['user_gate_mean'].numpy()),
        float(diagnostics['item_gate_mean'].numpy()),
        float(diagnostics['user_gate_saturation'].numpy()),
        float(diagnostics['item_gate_saturation'].numpy())))
