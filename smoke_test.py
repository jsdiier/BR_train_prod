...
def main():
    ...
    experiment_variables = [
        ...
        if 'domain_' in variable.name.lower()]
    experiment_missing = [
        variable.name for variable, gradient
        in zip(model.trainable_variables, gradients)
        if 'domain_' in variable.name.lower() and gradient is None]
    experiment_gradients = [
        variable.name for variable, gradient
        in zip(model.trainable_variables, gradients)
        if 'domain_' in variable.name.lower() and gradient is not None]
    if not experiment_variables:
        raise RuntimeError('experiment variables received no gradients: domain_')
    if experiment_missing:
        raise RuntimeError(
            'experiment variables with missing gradients: %s' %
            experiment_missing)

    if model.last_semantic_token_mask.shape[1] != 193:
        raise RuntimeError('semantic token count is not 193')
    diagnostics = model.rankmixer.diagnostics(
        model.last_semantic_token_mask)
    print('SEMANTIC_AUDIT ordinary_tokens=187 sequence_tokens=6 total_tokens=193 token_dim=256 active_tokens=%.4f' % (
        float(diagnostics[2].numpy())))
