...
def main():
    ...
    decayed, excluded = model.optimizer.decay_audit(model.trainable_variables)
    if any(any(token in name.lower() for token in (
            'embedding', 'bias', 'beta', 'gamma')) for name in decayed):
        raise RuntimeError('AdamW decay whitelist contains forbidden variable')
    gradient_by_name = {
        variable.name: gradient for variable, gradient
        in zip(model.trainable_variables, gradients)}
    decay_missing = [
        name for name in decayed if gradient_by_name.get(name) is None]
    if decay_missing:
        raise RuntimeError(
            'AdamW-decayed variables with missing gradients: %s' %
            decay_missing)
    experiment_gradients = decayed
    print('ADAMW_AUDIT decayed=%d excluded=%d' % (
        len(decayed), len(excluded)))
