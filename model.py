...
class Model(tf.keras.Model):
    ...
    def _encode_jz_v3_sid_sequences(self, pooled_output, slot_mask):
        """Return exactly two aligned SID sequence tokens: click and pay."""
        candidate_indices = self.slot_id_table.lookup(
            tf.constant(model_conf.jz_v3_sid_candidate_slots, dtype=tf.int32))
        query = tf.gather(pooled_output[:, :, 1:], candidate_indices, axis=1)
        query = tf.reshape(query, [tf.shape(query)[0],
                                   len(model_conf.jz_v3_sid_candidate_slots) * model_conf.fm_emb_size])
        query = self.jz_v3_sid_query_proj(self.jz_v3_sid_query_ln(query))
