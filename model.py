...
from module.finalmlp_fusion import FinalMLPTwoStreamFusion
...
class Model(tf.keras.Model):
    def __init__(self, training=False, pred=False, fid_kv=None, fid_ads_kv=None, l2_reg=0.0001):
        ...
        self.finalmlp_two_stream = FinalMLPTwoStreamFusion(
            stream_dim=256, gate_hidden_dim=128,
            name='finalmlp_two_stream')
        # 初始化序列网络
    ...
    def call(self, inputs, training=None):
        ...
        user_stream_input = tf.concat([emb_user] + seq_outputs, axis=-1)
        item_stream_input = tf.concat([emb_shop, emb_interact], axis=-1)
        two_stream_output = self.finalmlp_two_stream(
            [user_stream_input, item_stream_input])

        concat = tf.concat(
            [lr, fm, rankmixer_output, two_stream_output], axis=1)
