...
class Model(tf.keras.Model):
    def __init__(self, training=False, pred=False, fid_kv=None, fid_ads_kv=None, l2_reg=0.0001):
        ...
        # Late-arriving features share the existing FM embedding table, then
        # are summarized by semantic group before entering the task towers.
        ...
        self.interest_stat_indices = tf.constant(
            [all_slot_position[sid] for sid in model_conf.interest_stat_slot_ids],
            dtype=tf.int32)
    ...
    def encode_new_interest_features(self, pooled_output, slot_mask):
        """Encode forty masked interest groups and eight statistic slots."""
        ...
        stat_vector = tf.reshape(
            stat_emb,
            [tf.shape(stat_emb)[0],
             len(model_conf.interest_stat_slot_ids) * model_conf.fm_emb_size])
        ...
        residual = self.interest_fusion_residual(hidden) * has_new_feature
    ...
    def call(self, inputs, training=None):
        ...
        tf.debugging.assert_equal(
            tf.shape(concat)[1],
            model_conf.interest_fusion_output_dim,
            message='interest residual dimension must match task-tower input')
