    ...
    with open(path, "r", encoding="utf-8", errors="replace") as handle:
        for row in csv.DictReader(handle, delimiter="\t"):
            ...
            rows[key] = {
                ...
                "test_start_day": row["test_start_day"],
                "test_end_day": row["test_end_day"],
                "point_type": "rolling" if row["test_start_day"] == row["test_end_day"] else "fixed",
                "auc": float(row["auc"]),
                "size": int(row["size"]),
                "pos": int(row["pos"]),
            }
...
def validate_alignment(data):
    ...
    for name in names[1:]:
        ...
        for key in sorted(base_keys):
            ...
            cur = data[name][key]
            if (cur["checkpoint_day"], cur["test_start_day"], cur["test_end_day"],
                    cur["point_type"], cur["size"], cur["pos"]) != (
                    base["checkpoint_day"], base["test_start_day"], base["test_end_day"],
                    base["point_type"], base["size"], base["pos"]):
...
def build_payload(data):
    ...
    base_rows = data[names[0]]
    point_meta = []
    for day in dates:
        sample = next(base_rows[(day, task)] for task in TASKS if (day, task) in base_rows)
        is_fixed = sample["point_type"] == "fixed"
        point_meta.append({
            "type": sample["point_type"],
            "label": ("%s～%s（固定窗口）" % (sample["test_start_day"], sample["test_end_day"]))
                     if is_fixed else "%s（单日滚动）" % day,
            "axis_label": ("%s-%s\n固定" % (sample["test_start_day"][4:], sample["test_end_day"][4:]))
                          if is_fixed else "%s-%s" % (day[4:6], day[6:]),
        })
    payload = {"names": names, "dates": dates, "pointMeta": point_meta, "tasks": {}}
    for task in TASKS:
        series = {}
        for name in names:
...
<p>第一个点为固定测试窗口汇总AUC，后续点为测试日前最新真实checkpoint在单日数据上的滚动AUC；汇总统计只计算实际存在的单日滚动点。差值单位为千分位（‰）。</p>
