...
ONETRANS_NS_TOKEN_ORDER = [
    ...
    "candidate_match", "period_shop_cross", "weekday_shop_cross", "city_shop_cross", "device_shop_cross",
    "device_user_cross",
]
...
NEW_CITY_SHOP_CROSS_SLOTS = list(range(1530, 1550))
...
def validate_semantic_domain_schema():
    ...
    if set(NEW_CITY_SHOP_CROSS_SLOTS) != set(DOMAIN_SLOT_GROUPS['city_shop_cross']):
        raise ValueError("city_shop_cross must be exactly slots 1530..1549")
    if set(DROPPED_LEGACY_SLOTS).intersection(owner):
        raise ValueError("dropped legacy slots must not appear in semantic domains")
    return owner


ORDINARY_SLOT_OWNER = validate_semantic_domain_schema()
ORDINARY_SLOT_IDS = sorted(ORDINARY_SLOT_OWNER)
SEMANTIC_TOKEN_COUNT = len(ONETRANS_NS_TOKEN_ORDER) + len(SEQUENCE_TOKEN_ORDER)
