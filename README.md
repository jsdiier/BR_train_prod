...
## Experiment: merge all three JZ-v3 feature sources

This branch keeps the complete `BR_train_prod_bs_lr_ema_weights` configuration
and registers all feature-list slots 3001-3027 from initialization. From
20260818 it consumes two aligned native Shop sequences, five candidate Shop SID
features, and two aligned fused SID history sequences. SID history uses
`emb_shop` as query, which includes candidate SID only in this merged arm.
Evaluation runs through 20260906 and requires 4 fixed plus 24 rolling rows.
